...
class git_clones_counter(object):
    def __init__(self, reponame, username=None, password=None):
        ...
        self.repo=reponame
        if not os.path.exists('data/'+self.repo+'_clones.pkl'):
            self.initialize_repo()
        self.clones={}
        self.get_previous_clones()

    def initialize_repo(self):
        if os.path.exists('data/'+self.repo+'_clones.pkl'):
            os.remove('data/'+self.repo+'_clones.pkl')
        pickle.dump({},open('data/'+self.repo+'_clones.pkl','w'))

    def get_previous_clones(self):
        self.clones=pickle.load(open('data/'+self.repo+'_clones.pkl','rb'))
    ...
    def parse_clones_json(self,json_output):
        ...
        if len(self.clones.keys()) >= len(pickle.load(open('data/'+self.repo+'_clones.pkl','rb'))):
            pickle.dump(self.clones,open('data/'+self.repo+'_clones.pkl','wb'))
...
class badge_creator(object):
    def __init__(self,repo,username=None,password=None):
        self.clones=git_clones_counter(repo,username,password)
        new_clones=self.clones.get_git_clones_json()
        print(str(len(new_clones.json().keys()))+ " Number of new entries")
